		...
		else if (m_page_ok)
		{
			...
			for (unsigned int i = 0; i < elements; ++i)
			{
				...
				if (!element.m_text.empty())
				{
					...
					painter.renderText(area, element.m_text, gPainter::RT_WRAP|gPainter::RT_VALIGN_BOTTOM|rt_halignment_flag);
				}
			}
		}
...
//					text = (std::string) gRGB(255,255,0) + text;
				...
				painter.renderText(area, text, gPainter::RT_WRAP|gPainter::RT_VALIGN_BOTTOM|rt_halignment_flag);
			}
		}
...
void eSubtitleWidget::setFontStyle(subfont_t face, gFont *font, int haveColor, const gRGB &col, const gRGB &shadowCol, const ePoint &shadowOffset)
{
	...
	subtitleStyles[face].border_color = shadowCol;;
	subtitleStyles[face].border_width = 3;
